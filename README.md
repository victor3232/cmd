...
# BACKUP PTERODACTYL 
```
mysqldump -u root -p panel > /root/db_panel.sql
```
```
cp /var/www/pterodactyl/.env /root/env_panel.txt
```
# RESTORE PTERODACTYL 
```
mysql -u root -p panel < /root/db_panel.sql
```
```
Pindahkan APP_KEY dari .env Lama
Buka file env_panel.txt yang tadi abang bawa dari VPS lama. Cari baris yang tulisannya begini:
APP_KEY=base64:xxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
Copy kodenya. Terus buka file .env di VPS baru (/var/www/pterodactyl/.env), hapus APP_KEY yang baru, dan paste APP_KEY yang lama dari VPS sebelumnya.
```
```
...
# BACKUP DB UBOT
```
sqlite3 MDatabase.db ".backup Database_Aman.db"
```
```
rm Database_Aman.db
```
